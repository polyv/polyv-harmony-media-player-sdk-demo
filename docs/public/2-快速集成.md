...
为了播放您账号下的点播视频，播放器需要完成点播账号鉴权。基础接入可使用点播系统中的`userId`等账号参数，您可以登录点播后台，在[API 接口](https://my.polyv.net/secure/setting/api)中获取。

出于安全考虑，不建议将`secretKey`明文写入客户端代码或 APP 配置文件。生产环境更建议将`secretKey`保存在您的服务端，由服务端生成播放或下载所需的 token 后下发给客户端。播放场景请参考[4-加密视频版权保护](./4-加密视频版权保护.md)中的“配置传入token”，下载场景请参考[5-视频下载](./5-视频下载.md)中的“自定义传入token”。
